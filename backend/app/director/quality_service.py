...
class DirectorQualityService:
    ...
    async def review_trial(
        ...
        *,
        ...
        if decision == "accept":
            ...
            await self._promote_trial_canonical_set(batch=batch)
    ...
    async def _promote_trial_canonical_set(self, *, batch: ProductionBatch) -> None:
        """Promote every CharacterBible reference generated by the accepted trial."""
        raw_bible_id = batch.locked_version_refs.get(ArtifactKind.CHARACTER_BIBLE.value)
        if raw_bible_id is None:
            raise ConflictError("accepted trial has no locked CharacterBible")
        bible_version = await self._session.get(CreativeArtifactVersion, UUID(raw_bible_id))
        if bible_version is None or bible_version.project_id != batch.project_id:
            raise ConflictError("accepted trial CharacterBible is missing")
        bible = CharacterBiblePayload.model_validate(bible_version.payload)
        rows = list(
            (
                await self._session.execute(
                    ...
                    .where(
                        ...
                        GraphNode.node_key.in_(
                            [f"character_{item.character_id}" for item in bible.characters]
                        ),
                    )
                )
            ).tuples()
        )
        by_key = {node.node_key: (run, artifact) for run, node, artifact in rows}
        missing = [
            item.name
            for item in bible.characters
            if f"character_{item.character_id}" not in by_key
        ]
        if missing:
            raise ConflictError(
                "accepted trial does not cover the complete Canonical cast",
                details={"characters": missing},
            )
        for item in bible.characters:
            run, artifact = by_key[f"character_{item.character_id}"]
            await promote_project_character_canonical(
                self._session,
                project_id=batch.project_id,
                name=item.name,
                locked_prompt=item.locked_prompt,
                artifact=artifact,
                source_run=run,
            )
