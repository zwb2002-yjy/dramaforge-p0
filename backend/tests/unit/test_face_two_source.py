...
async def test_same_image_passes_and_different_blocks() -> None:
    ...
    same = face_review_images(
        probe_image_bytes=ba, canonical_image_bytes=ba, threshold=0.5
    )
    assert same.status == "passed"
    assert same.score is not None and same.score > 0.9
    diff = face_review_images(
        probe_image_bytes=bb, canonical_image_bytes=ba, threshold=0.95
    )
...
async def test_missing_canonical_blocked() -> None:
    ...
    ba = ad.blobs[a["remote_task_id"]]
    out = face_review_images(
        probe_image_bytes=ba, canonical_image_bytes=b"", threshold=0.35
    )
