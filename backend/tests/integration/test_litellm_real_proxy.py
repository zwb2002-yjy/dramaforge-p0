...
async def test_text_v3_bridge_brief_against_real_proxy(
    real_proxy: str, monkeypatch: pytest.MonkeyPatch
) -> None:
    """F14: TEXT_V3_ROUTER_ENABLED=true routes Agent Brief through the real
    Proxy (DramaForge → CapabilityRouter → LiteLLMModelAdapter → REAL Proxy →
    mock deployment). Persists a succeeded ProviderOperation on the bridge."""
    ...
    monkeypatch.setenv("LITELLM_GATEWAY_URL", real_proxy)
    ...
    async with factory() as session:
        user = User(
            email="int-bridge@example.com",
            display_name="B",
            password_hash=hash_password("password123"),
        )
        session.add(user)
        await session.flush()
        workspace = Workspace(owner_user_id=user.id, name="Int-W")
        ...
        ops = (
            await session.execute(
                select(ProviderOperation).where(
                    ProviderOperation.agent_run_id == agent.id
                )
            )
        ).scalars().all()
        assert len(ops) == 1
