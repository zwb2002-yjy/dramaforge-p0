...
**当前事实：产品方向、目标用户和阶段顺序以 `DramaForge总开发文档.md` 为准；当前真实代码、未关闭 Gate 和唯一下一任务以 `docs/开发执行检查点.md` 为准。**
...
1. 用户当前明确指令。
2. [`DramaForge总开发文档.md`](DramaForge总开发文档.md)：项目唯一总纲，定义产品方向、目标用户、壁垒、阶段路线和进入 Gate。
3. [`docs/README.md`](docs/README.md)：文档职责、状态语义和阅读顺序。
4. 当前阶段正式合同。P0 为 [`01_项目总需求.md`](01_项目总需求.md) 至 [`06_受控混合Agent运行时规范.md`](06_受控混合Agent运行时规范.md)；P1/P2 必须先建立版本化阶段合同。
5. [`AGENT_EXECUTION_PROTOCOL.md`](AGENT_EXECUTION_PROTOCOL.md)：本地记录、多 subagent、Git 分支/worktree、GitHub PR 和中断恢复规则。
6. `.agent-control/PROGRESS.jsonl` 尾部与 `open` 结果：本机执行断点；不属于产品运行时，也不进入 Git。
7. [`docs/开发执行检查点.md`](docs/开发执行检查点.md)：实际进度、已完成 Gate、阻塞和唯一下一步。
8. 当前 Task 明确引用的 ADR、测试、运行手册、产品路线图和延期台账条目。
9. [`AI短剧工作台完整实施规划.md`](AI短剧工作台完整实施规划.md) 及其他历史/专题材料：只用于追溯，不得覆盖总纲和当前合同。

默认禁止把历史汇报、历史审计、专题研究和 `D:\项目` 下的源资料当成实现合同。只有当前
Task 明确需要解释决策背景时才可只读打开；它们不能直接产生实现任务、目录、接口或排期。
...
- 富故事板、审核/返工队列、正式资产库和模板复用：个人创作者 P1.2。
- 受控精剪、FCPXML、OpenCut：P1.3；OpenCut 还必须先通过独立 Spike。
- 成员、邀请、共享、评论、任务指派、完整团队协作和实时共同编辑：当前路线未排期，未来如需实现必须重新批准产品范围和授权模型。
...
P0 完成后无需等待新的开发指令。主 Agent 依次启动个人创作者路线的 `P1.1-CONTRACT`、
`P1.2-CONTRACT`、`P1.3-CONTRACT`、`P2-CONTRACT`：把产品路线中已经批准的阶段效果
转换为正式 PRD、受影响合同清单、ADR、迁移/API/Command 计划、fixture 和验收测试。
不得把 P0 的 `01`–`06` 直接改写成所有阶段共用合同。若出现会改变产品方向的多种合理
解释，特别是重新引入多人协作时，必须询问用户。
...
1. P1.1：候选结果治理与项目内复用。
2. P1.2：个人故事板、审核/返工队列、正式资产库和模板复用。
3. P1.3：版本化时间线、受控精剪、FCPXML；OpenCut 只作为通过 Spike 后的可选 Adapter。
4. P2：3D 导演台、高级预演和更专业的后期互操作。

成员、邀请、共享、评论和任务指派不在上述自动路线中。
