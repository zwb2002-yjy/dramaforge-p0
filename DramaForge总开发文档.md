...
**代码审计基线：`dev@3c8f795494ea9b7cbe0d72652fe4b9ca8a38ca99`**
...
产品的核心体验不是“接了多少模型”，也不是“InsightFace 分数多高”，而是用户能否：
...
| 开源 | 产品能力开源；未来收入来自企业私有部署、适配、定制与支持，许可证在 RC 前完成决策 |
...
## 6. 人物一致性与 InsightFace 结论

InsightFace 只能作为“可见且可检测的人脸是否相似”的一个信号，不能判断发型、服装、体型、气质、全身镜头、遮挡、多人身份归属或动态漂移，也不能证明模型请求实际使用了参考图。因此：

- 现有 `0.60` 不能继续作为发布级统一硬门槛；
- 第一优先级是记录并验证参考图、权重和高级参数是否进入供应商的**有效请求**；
- 人物一致性必须由请求完整性、静态身份/外观、视频时序、声音/口型、剧情连续性和人工验收共同判断；
- 阈值必须在 DramaForge 自建并人工标注的真实镜头集上校准，且保留模型、镜头类型与置信区间；
- 主观质量允许用户查看证据后覆盖，文件损坏、缺失、安全与授权等硬错误不可覆盖。

此外，InsightFace **代码**可按其开源许可使用，不代表其官方预训练权重可直接商用。发布版不得捆绑或自动下载授权不清晰/仅限非商业研究的权重；必须使用有商业授权或许可证兼容的替代模型，并生成第三方组件清单。
...
- 开源许可证及商业双许可方式：最迟 2026-09-12 决定；
