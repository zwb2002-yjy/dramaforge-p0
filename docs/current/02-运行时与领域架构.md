...
> 本文中出现的预算、价格、费用和计费字段均为旧快速模式/历史数据兼容说明，不属于专业工作台执行路径。专业版由模型 Provider 负责价格与结算，DramaForge 只保存必要的 Provider 原始审计信息。
...
AI 导演负责理解目标、整理上下文、解释方案、提出结构化变更和在允许的分支中做选择。确定性系统负责权限、状态迁移、版本、任务执行、产物血缘和质量 Gate。

> **专业版覆盖说明（2026-08-25）**：专业工作台取代四阶段向导成为默认体验。场景、镜头、资产和 Canvas Revision 是产品事实源；四阶段 Director Workflow 作为兼容入口和受控执行编排继续存在，不再定义前端主交互。

> **预算/计费边界（2026-08-25）**：预算授权、价格估算和计费展示不属于专业版产品体验。ProviderOperation 的成本字段、旧 Director budget 命令和预算表仅为兼容旧快速模式、内部审计和历史数据保留；专业工作台执行由用户确认、能力校验、质量 Gate 和现有运行时状态控制。
...
| 层 | 职责 | 主要实现 |
|---|---|---|
| 体验层 | 场景/镜头专业工作台、导演助手、快速兼容模式、Provider 设置 | `frontend/src/routes`、`frontend/src/features`、`frontend/src/components` |
| API / Application | 身份、业务命令、聚合 Snapshot、下载授权；不直接执行长任务 | `backend/app/api` |
| 创作领域 | 创意、剧本、角色、分镜、版本、确认、试拍和修复决策 | `backend/app/director`、`backend/app/creation`、`backend/app/assets` |
...
- Worker 只信任 Run ID，必须从数据库重建 Project、Actor、Binding 和版本上下文；不读取或执行专业版预算 Gate。
...
| Provider 计费原始信息 | ProviderOperation（只读审计） | DramaForge 自建账单、预算表或前端估算 |
...
```mermaid
flowchart TB
  UI["专业工作台 + AI 导演"] --> DW["Director Workflow\n项目级创作与确认"]
  DW --> SK["Skill Runtime\n版本化原子能力"]
...
- 专业画布状态、版本与建议确认；快速模式的四个硬确认点作为兼容规则保留；
- Issue、变更提案、影响分析、锁定和回退；
- 风险报告、模型 Selection Plan、Provider 能力与预计耗时；
- 试拍授权、试拍验收与正式生产授权；
- 修复方案选择和生产图局部重物化。
...
LLM 输出只能是动作提案；Command Handler 再校验当前状态、Actor、版本前置条件和幂等键；Provider 负责价格与结算。
...
本表基于 2026-08-19 的 `dev@acaa6c4` 审计，模块路径描述当前实现边界，不代表真实 Provider、用户或发布 Gate 已通过：
...
| `backend/app/providers/` | Intent、Manifest、Selection、Compiler、Runtime、参考交付与多 Provider 已实现；Director 图片/视频强制 Unified，非 Director 历史兼容路径按恢复合同保留 |
| `backend/app/providers/model_profiles/` | Production Profile 已提供版本化 Slot 绑定；继续作为选择快照与能力档案，不另造平行配置系统 |
| `frontend/src/routes/projects.$projectId.quick.tsx` | Visual 2.0 已进入 canonical 快速创作路由，并继续使用真实 Director API、四阶段状态机和共享项目事实 |
| `frontend/src/routes/projects.$projectId.production.tsx` | 专业生产板已接入版本、质量、局部修复和交付事实；当前 Chromium Mock E2E 11 项及真实 Provider 三镜生产、验收、导出全链均已完成 |
...
- Director 已只有 Unified Media Path；非 Director 历史兼容路径按恢复合同保留，不作为 Director fallback；
- 项目级 Canonical Reference Set 已由三个真实 keyframe 请求证明复用同一 Artifact ID/SHA-256；
- EffectiveRequest / TranslationReport 已有 Agnes 真实 I2I/I2V ProviderOperation 与 Artifact 证据；
- 试拍页面直接展示视频、音频和时序帧证据；
...
3. 输入输出版本和事实源是什么？
4. 哪个状态允许执行，谁授权？
5. Provider 是否返回计费原始信息，如何保留审计引用？
...
| ID | 要证明什么 | 最小验收场景 | 必须断言 | 当前事实（`dev@faee665`） | 状态 | 下一关闭动作 |
|---|---|---|---|---|---|---|
| AP-01 | 快速/专业模式共用一个事实源 | 快速模式确认 StoryCore 新版本，专业模式读取同一 revision、批次和预算状态 | Project/版本 ID 相同；无复制 Artifact；旧批次与预算血缘不丢失 | Visual 2.0 canonical 路由与专业生产板共享真实 Project/Director API；当前候选 Chromium Mock E2E 通过，尚缺 authenticated live-stack 重验 | `PARTIAL` | 在当前干净候选重跑 authenticated live-stack 场景 |
...
| AP-03 | Provider 插件化不是移动硬编码 | 同一 I2V Intent 分别编译为 Agnes、MiniMax 或 Volcengine 请求 | 业务层无模型名分支；原生 payload 不同；不支持参数 fail-closed；保存 EffectiveRequest/TranslationReport | Director 真实 t2i/I2I/I2V 均经 Frozen Binding、Manifest、Compiler、Runtime 和 Unified ProviderOperation；Worker 以持久化远端 ID 恢复；非 Director 历史兼容不进入 Director | `PASS` | 保持边界测试，后续只在有迁移替代时删除非 Director 历史恢复代码 |
| AP-04 | 预算与付费请求边界真实有效 | 无授权、过期授权、低于估算和有效授权四种情况 | 前三种 Provider POST 为 0；有效授权只覆盖冻结 Binding、价格、镜头和次数；实际费用结算可追溯 | 试拍、修复和正式生产分别授权；真实付费媒体 operation 均 attempt 1、无重复 submit；Provider 未报告费用时账本保持 `not_reported` | `PASS` | 继续等待 Provider 账单口径，不用未知金额替换 `not_reported` |
| AP-05 | 局部修复真的只影响必要范围 | 一个真实视频镜头因质量问题被拒绝，选择“复用关键帧，仅重试视频” | 只新建 video、video_drift_review、composite、continuity_review；复用 keyframe/voice/subtitle；额外预算单独授权 | Repair Batch `933ee4bc…` 在独立 10 USD 授权下完成；仅 video 与三个下游为新 run，Canonical/Keyframe/Voice/Subtitle 复用，旧失败保留；真实执行候选为 `214a527`，不是当前 `faee665` | `PARTIAL` | 在冻结 RC 的目标用户流程中自然复现一次局部修复，不为补证单独重复付费 |
| AP-06 | 整体架构能交付真实用户价值 | 1 个主角、3 个镜头、15–25 秒的完整作品 | 完成四阶段、四确认、真实试拍、逐镜验收、至少一次局部修复和四项交付；所有证据同一 commit | 已完成 15.17 秒三镜黄金样本和四项交付；当前 Production 候选为 `faee665`，但试拍/修复证据跨候选恢复，且三名目标用户尚未执行 | `PARTIAL` | 以当前稳定入口完成三名目标用户无人代操作验证，再冻结单一 RC 证据集 |
