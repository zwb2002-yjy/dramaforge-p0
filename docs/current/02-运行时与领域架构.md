...
| `backend/app/providers/` | Intent、Manifest、Selection、Compiler、Runtime、参考交付与多 Provider 已实现；Director 图片/视频强制 Unified，非 Director 历史兼容路径仍待真实 Agnes 验收后清理 |
...
| `frontend/src/routes/projects.$projectId.production.tsx` | 专业生产板已接入版本、质量、局部修复和交付事实；当前 Chromium Mock E2E 11 项通过，仍缺真实 Provider 全链 |
...
- Director 已只有 Unified Media Path；待真实 Agnes I2I/I2V 和恢复验证后清理非 Director 历史兼容路径；
- 项目级 Canonical Reference Set 已完成试拍生成、接受提升和正式生产缓存复用；待真实请求证明三个镜头使用同一 Artifact 哈希；
- EffectiveRequest / TranslationReport 的 Spy 证据已完整，仍缺 Agnes 真实 Provider 证据；
- 试拍页面直接展示视频、音频和时序帧证据；
...
| ID | 要证明什么 | 最小验收场景 | 必须断言 | 当前事实（`dev@acaa6c4`） | 状态 | 下一关闭动作 |
|---|---|---|---|---|---|---|
...
| AP-03 | Provider 插件化不是移动硬编码 | 同一 I2V Intent 分别编译为 Agnes、MiniMax 或 Volcengine 请求 | 业务层无模型名分支；原生 payload 不同；不支持参数 fail-closed；保存 EffectiveRequest/TranslationReport | Director 强制 Unified；G1/G2/G3 通过；Agnes 图片 v2 和视频严格子集、请求证据与引用匹配已有 Spy/Compose 证明；非 Director 历史兼容路径仍保留 | `PARTIAL` | 完成 Agnes 一次真实 I2I/I2V 与 Worker 恢复，再清理历史兼容入口 |
