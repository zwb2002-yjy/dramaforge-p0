...
只在需要合同/账本操作、恢复已有 Task、Git/worktree、跨域验证或正式发布交接时读取对应章节。简短入口是 [AGENT_EXECUTION_PROTOCOL.md](../../../AGENT_EXECUTION_PROTOCOL.md)，任务类型先按 [agent.md](../../../agent.md) 判定。

- 下方连续循环、重算 READY、下一 Task 与 Goal 最终回报，只用于已授权的连续 Goal；不为只读任务写账本，也不为独立任务强行生成后续产品工作。
- 命令和路径以仓库根为基准，执行前核对当前脚本参数和合同；只加载与所需操作有关的章节。
- 发布依据由[七方案执行集及最新 Owner amendments](../../plans/professional-program-v2/README.md)和当前 Task 指定。原文指向的旧 Release Board 只作历史证据，不能自行决定当前 Gate、产物位置或完成声明。
- 旧版“COMPLETED 条件”中的“下一 READY Task 已计算”仅适用于 Goal 内的 Task。其余合同、commit、证据与账本条件仍保留；尚未提交的改动不能伪记为正式 COMPLETED。
- 下方是优化前 v4.0 协议完整原文，不增加产品范围、权限或新的发布门禁。

<!-- preserved-source: AGENT_EXECUTION_PROTOCOL.md; sha256=91d53056f5fd2d7f9fcd1c4d16a0cdfd19f4678e23ce32c454bf0eec0bcf2d71 -->
