...
仅在启动/恢复已授权的 V1 Goal，或当前 Task 确实需要某项 Goal 约束时，按章节读取。它不是所有会话的启动清单，也不是第二份产品规划。

- 当前请求的范围与读序由 [agent.md](../../../agent.md) 决定；产品依据仍由[七方案执行集及最新 Owner amendments](../../plans/professional-program-v2/README.md) 决定。
- 下方“每次启动”“自动进入下一 Task”和付费授权只适用于明确处于执行中的对应 Goal，不将只读审计或独立修复扩展为整个 Goal，也不向其他任务授予费用或远端操作权限。
- 原文的启动 SHA、PR、G0 起点是编制时的基线，不代表现在的进度；恢复位置由当前合同与实际证据确定。发生 Owner revision 时，以现行权威为准，不复制另一份规划。
- 下方文件/命令路径沿用仓库根相对路径。此处保留优化前 agent.md 的完整原文，以便按需追溯与执行；无需从头通读所有章节。

<!-- preserved-source: agent.md; sha256=50fc84f028fc0836559406f6c904e337011d54cedbb874b0b189f07cfbde0d41 -->
