...
**Status:** LIVE / operational preflight, not product billing or spend authorization
...
replace them and does not authorize product or runtime changes.
...
This document never creates a DramaForge budget, billing record, reservation,
or execution gate. A real submission is permitted only after the Owner or
delegated authorizer gives separate written approval for the exact run scope.
A successful API response alone does not close Q1, A3, or A4: the approval,
frozen binding, effective-request translation, reference lineage, and
sanitized post-run evidence must all be present. Provider-reported cost, when
returned, is retained only as raw execution audit metadata.
...
| Role | Responsibility |
|---|---|
| Owner / delegated authorizer | Gives written per-run authorization for a narrowly defined scope. |
...
5. The capability manifest confirms the required modality, aspect ratio,
   duration, reference mechanism, native-audio behavior, and requested advanced
   parameters.
6. Every required Canonical, first-frame, or other reference Artifact is
   readable, allowed for this binding, and has a recorded immutable hash.
7. The model, weight, voice, and other resource license inventory is complete
...
must contain only `authorization_id`, role, date, expiry, scope, and retry
rule; it must not contain a name, signature image, credentials, cost data, or a
payment instrument.
...
binding, or retry outside the stated attempt rule requires new written
authorization and a new preflight decision.
...
4. Save `capability-evidence.json`: capability manifest ID/hash and relevant
   declared slots/constraints. Do not include a secret endpoint URL or account
   balance.
...
execution records; do not manually recreate a request in a Provider console
...
a required reference is unavailable, or the license inventory is incomplete.
Record the blocker code and safe diagnostic only.

Set the run to `ABORTED` after submission when the stop condition is reached,
the response makes submission state unknown, an unexpected binding is observed,
or a required evidence item cannot be safely retained. Do not issue an
additional retry without fresh authorization. Preserve the evidence gathered so
far with the real outcome.
...
| `operation-summary.json` | ProviderOperation ID, redacted request/response identifiers, submission status, latency, optional raw provider-reported audit field, and sanitized compiled request summary. |
...
```markdown
| YYYY-MM-DD | Q0, Q1, A3, A4 | OPEN/PARTIAL -> PARTIAL/PASS | <full commit> | tmp/p0-evidence/<commit>/real-provider/ | <reviewer role> | Authorization <opaque ID>; scope <scope>; result <pass/blocked/aborted>; known limits <summary>. |
```
