...
- `--df-*` Token 是颜色、排版、间距、圆角、阴影、时长、层级的唯一来源，
  禁止硬编码；
- 主交互色 verdigris Token（浅紫，主操作 / active / progress），注意色 brass Token（暖杏）
...
- 排版沿用 11/12/13/14px 主刻度；控件/容器/顶层外壳圆角统一为 12/20/24px。
...
| 创作 | 剧本 → 资产 → 场景 → 制作（含待审）→ 剪辑 | 导演参与度随当前创作项目显示；创建时设初值，创作中可调整 |
...
- 非首页在内容区提供返回入口，不依赖二级侧栏或浏览器历史：大厅子页回大厅，镜头页回场景，待审回制作，设置返回经过校验的来源（无来源回大厅），项目模型回模型连接。
...
- 下一步卡片只保留一个主操作；已有页签可达的待审内容不再重复一个按钮。
...
全产品共用中性炭灰画布、浅紫主操作与暖杏内容强调，具体配方见 `frontend/design/README.md`。范围包括大厅、双层导航、设置、剧本、资产、场景、制作、审片、剪辑；不改变单创作主链与显式保存门。纯结构容器以留白分层，操作对象保留轻边界与焦点提示，媒体预览维持中性黑底。共鸣舞台保留原有表现例外。
