...
**状态**：HISTORICAL / 已由 `DramaForge总开发文档.md` 取代

**版本**：v2.0

**编制日期**：2026-07-10

**最近同步**：2026-08-12（降级为历史设计底稿）

**项目代号**：DramaForge

**产品定位**：面向个人短剧创作者、支持私有化部署的镜头级 AI 生产工作台

> 当前产品方向、目标用户、壁垒、完成状态和开发顺序统一以
> [`DramaForge总开发文档.md`](DramaForge总开发文档.md) 为准。本文保留 2026-07 至
> 2026-08-04 的规划、调研和详细设计，只用于追溯，不再回答“当前下一步是什么”。
>
> 本文原先回答“DramaForge 整体要做成什么、当前在哪个阶段、下一步开发什么”。
> 根目录 `01_项目总需求.md` 至 `06_受控混合Agent运行时规范.md` 只负责 P0
> 的冻结实施合同，不替代本项目总规划。P0 的具体实现发生冲突时，以 `01`–`06`
> 对应主题为准；P1/P2 开始前必须再建立该阶段的正式合同，不能直接照抄本文后半部的
> 早期代码和排期示例。

## 0. 历史可执行总览（2026-08-04）

### 0.1 产品方向

DramaForge 当前只面向**个人创作者**：

```text
个人账号
  -> 一个或多个私有 Workspace
    -> Project
      -> 剧本 / 角色与资产 / Shot / 生成 / 审核 / 返工 / 交付
```

- 快速出片模式与专业工作台模式是同一 Project 的两种入口，共用资产、Production
  Graph、Run、成本、审核和交付事实。
- 产品核心是“创作者可控地完成一部短剧”，不是多人项目管理，也不是一次性黑盒生成。
- 当前路线不实现组织、成员、邀请、共享空间、项目角色、评论、任务指派或实时协作。
  多人协作不是默认 P1 阶段；若未来重新考虑，必须单独进行产品决策和授权模型设计。
- 私有化部署、Workspace 隔离、BYOK、失败关闭、成本可追溯和可重现交付继续保留。

### 0.2 当前阶段结论

| 项目 | 当前事实 | 结论 |
|---|---|---|
| 代码审计基线 | 本轮文档同步前为 `8299db6`；工作树随后会因文档同步而变脏 | 最终验收必须绑定同步后的干净提交，不能沿用该 SHA |
| P0 工程主体 | GraphEdge/依赖顺序、0.60 人脸 Gate、真实图像/视频链、本地 TTS、Drift 决策、审核阻断和四项交付均已有实现与定向证据 | 属于 P0 功能候选版 |
| 最近 10 Shot 事实 | 绑定 `211ecaa` 的干净运行中，10/10 keyframe、10/10 face、10/10 video；9/10 drift 和完整下游通过，shot 6 两次 drift 约 0.31 被阻断 | 真实链路接近闭环，但黄金样本仍有 1 Shot 不可交付 |
| 历史全绿 Gate | `a8feda0` 曾为 `25 PASS / 0 FAIL / 0 BLOCKED`，但早于当前实现 41 个提交，也早于 Drift 与审核 Gate 加固 | 只能证明历史能力，不能证明当前 P0 |
| 当前运行栈 | 本轮核对 `/health.source_commit=211ecaa`，早于当前代码基线 | 必须重建 Compose 后才可形成当前证据 |
| P0 状态 | 当前提交没有同源 formal、Playwright、Section 3.1 Gate 和 ops 全套报告，shot 6 仍待合格重跑 | **P0 尚未完成** |
| 下一阶段 | P1 会修改候选产物、故事板/资产和时间线合同 | **暂不进入 P1 功能开发**；可并行准备 P1.1 合同草案 |

### 0.3 P0 收口清单

以下条件全部满足后，才可把产品状态从“P0 功能候选版”改为“P0 MVP 完成”：

1. 对 shot 6 重跑 `video`，创建新的 attempt，并让新的 `video_drift_review` 实际
   `passed`；不得覆盖旧 Review，也不得用人工注记改写为通过。
2. 将当前代码和文档形成干净候选提交，重建 Compose，并确认 API、default/heavy
   Worker 的 `source_commit` 与候选提交完全一致。
3. 在同一候选提交上运行真实 10 Shot formal proof、真实 Playwright 主业务流、
   Section 3.1 Gate 和运维演练。
4. Gate 结果必须全部 `PASS`，不得有 `FAIL`、`BLOCKED`、dirty 或 source mismatch；
   MP4、SRT、素材包和 `timeline_json` 必须可下载、校验哈希并回链 90 个正式产物。
5. 完成备份恢复、BYOK 轮换、Outbox/死信、取消竞态、SSE 恢复和冷存储失败演练，
   并由产品负责人完成人工验收与发布批准。

### 0.4 已锁定的质量 Gate 决策

- `needs_human` 继续 **fail-closed**。它表示需要人查看证据并决定如何重跑，不表示可以
  人工把失败结论覆盖成 `passed`。P0 不新增 review override API。
- Video Drift 继续使用已批准的 `mean >= 0.40`（policy
  `P0-VIDEO-DRIFT-2026-08-04`）。每帧分数、最小/最大值和过阈值帧数用于增强证据，
  不改变当前判定规则；中位数或多数帧约束必须重新用真实样本校准后另行批准。
- shot 6 是已记录的人工复核项，但“人工复核”只负责诊断并触发新 video attempt；
  在新 attempt 通过前，它仍阻断 composite、continuity 和交付。

### 0.5 产品阶段路线

| 阶段 | 面向个人创作者的目标 | 开始条件 | 完成标志 |
|---|---|---|---|
| **P0** | 从创意/剧本到至少 10 Shot 的生成、审核、局部返工和四项交付 | 已在实施 | 当前提交的完整 P0 Gate 全绿并获发布批准 |
| **P0.5** | 剪映草稿等不阻断主链的兼容增强 | 对应 Spike 独立通过 | 目标工具真实导入和回归通过；失败不影响 P0 |
| **P1.1** | 多候选结果治理、选片、废卡保留和项目内复用 | P0 完成；候选领域合同获批 | 创作者可追溯地采用、拒绝、归档、恢复和提升候选 |
| **P1.2** | 个人故事板、审核/返工队列、正式资产库和模板复用 | P1.1 稳定 | 单人可高效管理几十至上百 Shot，不产生第二套执行真相 |
| **P1.3** | 版本化时间线、受控精剪和后期工具衔接 | P1.2 稳定；交付重现合同稳定 | 排序、裁剪、字幕、音量和有限转场可由 FFmpeg 重现 |
| **P2** | 高级导演预演、机位/走位/构图控制和专业后期互操作 | P1 稳定且领域 Spike 通过 | 导演决策进入输入哈希并可回滚，目标后期格式往返通过 |

P1 的默认开发顺序固定为 `P1.1 -> P1.2 -> P1.3`。多人协作不在这条路线中。

### 0.6 文档职责

| 需要回答的问题 | 唯一入口 |
|---|---|
| 整个项目做什么、现在在哪、后续怎么走 | 本文 |
| P0 必须实现什么 | `01_项目总需求.md` |
| P0 技术、目录、数据、模块和 Agent Runtime 合同 | `02`–`06` |
| 当前完成度、唯一执行任务和 P0 收口 Gate | `docs/开发执行检查点.md` |
| 各产品阶段的用户效果和准入条件 | `docs/产品阶段与效果路线图.md` |
| P0 外能力是否已批准 | `docs/MVP能力延期台账.md` |
| P0 真实 Provider 与验收步骤 | `docs/P0开发验收执行方案-Agnes中国站.md` |
| 重要决策及理由 | `docs/adr/` |
| 全部文档导航与状态定义 | `docs/README.md` |

---

# 历史详细设计底稿（2026-07-10）

> 以下内容完整保留早期调研、架构草图、Schema、API、成本和风险分析。它是项目规划的
> 设计来源，不是当前代码合同。其中团队定位、Python RQ、WebSocket、旧 Agent 写法、
> 无条件剪映草稿、DaVinci/EDL/AAF P0 承诺、12 周排期和成本估算均已被上面的当前总览、
> `01`–`06` P0 冻结包及现行 ADR 替代。
...
本历史部分继续作为完整规划中的设计来源保留，但不单独维护。后续实施、评审和 AI 编码
代理先阅读本文第 0 节与 [`docs/README.md`](docs/README.md)，再按当前阶段进入对应合同。
P0 实现必须遵从根目录 `01_项目总需求.md` 至 `06_受控混合Agent运行时规范.md`；双模式
历史汇报见 `DramaForge双模式产品与架构汇报方案.md`，架构取舍见
