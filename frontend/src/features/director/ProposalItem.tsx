  ...
  return (
    <div data-testid={`proposal-item-${item.id}`} className="rounded border p-3 text-sm">
      ...
      <dl className="mt-2 grid grid-cols-2 gap-1 text-xs">
        <div>
          <dt className="text-gray-500">原因</dt>
          <dd>{item.rationale || "—"}</dd>
        </div>
        <div>
          <dt className="text-gray-500">收益</dt>
          <dd>{item.benefit || "—"}</dd>
        </div>
        <div>
          <dt className="text-gray-500">创作代价</dt>
          <dd>{item.cost || "—"}</dd>
        </div>
        <div>
          <dt className="text-gray-500">风险</dt>
          <dd>{item.risk || "—"}</dd>
        </div>
        <div>
          <dt className="text-gray-500">影响范围</dt>
          <dd>{item.impact || "—"}</dd>
        </div>
      </dl>
      {item.status === "pending" && (
